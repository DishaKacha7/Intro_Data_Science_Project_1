...
flight_accidents_data<-read.csv("DataSet/Airplane Accidents Severity.csv")
str(flight_accidents_data)
```

```{r read_csv, include = TRUE}
library(ggplot2)
ggplot(flight_accidents_data) + 
  geom_bar(aes(Severity,Safety_Score, fill = as.factor(Severity)), 
           position = "dodge", stat = "summary", fun.y = "mean")

ggplot(flight_accidents_data) + 
  geom_bar(aes(Severity,Adverse_Weather_Metric, fill = as.factor(Severity)), 
           position = "dodge", stat = "summary", fun.y = "mean")
...
```{r read_csv, include = TRUE}
flight_accidents_data$Severity<-c(Minor_Damage_And_Injuries=1,Significant_Damage_And_Serious_Injuries=2,Significant_Damage_And_Fatalities=3,Highly_Fatal_And_Damaging=4)[flight_accidents_data$Severity]

library(ggcorrplot)
corr <- round(cor(flight_accidents_data), 1)
ggcorrplot(corr)
```

```{r read_csv, include = TRUE}
sample <- sample(c(TRUE, FALSE), nrow(flight_accidents_data), replace=TRUE, prob=c(0.7,0.3))
train <- flight_accidents_data[sample, ]
test <- flight_accidents_data[!sample, ]  

model <- glm(Severity~Safety_Score+Days_Since_Inspection+Turbulence_In_gforces, family="quasipoisson", data=train)
summary(model)
...
test$default <- ifelse(flight_accidents_data$Severity=="Yes", 1, 0)

#find optimal cutoff probability to use to maximize accuracy
optimal <- optimalCutoff(flight_accidents_data$Severity, predicted)[1]
optimal
