...
```
## Stacked Bar Plot: Severity vs. Accident Type

```{r stacked_bar_plot_severity_accident_type, fig.width=12, fig.height=8}
# Create a contingency table
contingency_table <- table(airplane$Severity, airplane$Accident_Type_Code)

# Create a stacked bar plot
barplot(contingency_table, 
        col = c('lightblue', 'lightgreen', 'lightcoral', 'lightpink', 'lightyellow', 'lightcyan', 'lightgrey'), 
        main = 'Stacked Bar Plot: Severity vs. Accident Type',
        xlab = 'Severity',
        ylab = 'Count',
        legend.text = TRUE,
        beside = TRUE)
...
present_levels <- levels(factor(airplane$Accident_Type_Code))

# Create a legend only for the present levels
legend("topright", legend = present_levels, fill = c('lightblue', 'lightgreen', 'lightcoral', 'lightpink', 'lightyellow', 'lightcyan', 'lightgrey'))
...
```{r}
glmfit <- glm(Severity ~ Safety_Score, data = training, family = binomial)
summary(glmfit)

ggplot(training, aes(x = Severity, y = Safety_Score)) + geom_point() +
  stat_smooth(
    method = "glm",
    formula = y ~ x,
    color = "blue",
